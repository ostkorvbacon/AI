...
class Suppressor:
    def __init__(self):
        ...
        topic_type = self.wait_for_topic(rospy.get_namespace() + lower_topic)
        if topic_type is None:
            rospy.logerr(f'Topic {lower_topic} not found')
            return
        
        self.pub = rospy.Publisher('~output', topic_type, queue_size=queue_size)
        self.start_time = rospy.Time()
        rospy.Subscriber(upper_topic, topic_type, self.upper_cb, queue_size=queue_size)
        rospy.Subscriber(lower_topic, topic_type, self.lower_cb, queue_size=queue_size)
